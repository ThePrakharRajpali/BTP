...
def initialize_glfw():
    ...
    glfw.window_hint(GLFW_CONSTANTS.GLFW_OPENGL_FORWARD_COMPAT,
...
class App:
    ...
    def handleKeys(self):
        ...
        if combo > 0:
            ...
            dPos = [
                self.frameTime * 0.025 *
                np.cos(np.deg2rad(self.scene.player.theta + directionModifier)),
                self.frameTime * 0.025 *
                np.sin(np.deg2rad(self.scene.player.theta + directionModifier)),
                0
            ]
